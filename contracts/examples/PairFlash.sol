...
import '../v3-core/interfaces/callback/IETCswapV3FlashCallback.sol';
...
/// @notice An example contract using the ETCswap V3 flash function
contract PairFlash is IETCswapV3FlashCallback, PeripheryPayments {
    ...
    function etcswapV3FlashCallback(
        uint256 fee0,
        uint256 fee1,
        bytes calldata data
    ) external override {
    ...
    /// @notice Calls the pools flash function with data needed in `etcswapV3FlashCallback`
    function initFlash(FlashParams memory params) external {
        PoolAddress.PoolKey memory poolKey =
            PoolAddress.PoolKey({token0: params.token0, token1: params.token1, fee: params.fee1});
        IETCswapV3Pool pool = IETCswapV3Pool(PoolAddress.computeAddress(factory, poolKey));
